...
public class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject baseJsonRespone = new JSONObject(QueryUtils.SAMPLE_JSON_RESPONSE);
            JSONArray featuresArray = baseJsonRespone.getJSONArray("features");

            for(int i=0 ; i< featuresArray.length() ; i++){
                JSONObject currentEarthquake = featuresArray.getJSONObject(i);
                JSONObject properties = currentEarthquake.getJSONObject("properties");

                earthquake = new Earthquake(properties.getDouble("mag"),properties.getString("place"),
